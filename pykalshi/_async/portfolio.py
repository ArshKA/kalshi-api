...
class AsyncPortfolio:
    ...
    async def amend_order(
        ...
        *,
        ...
        response = await self._client.post(endpoint, body)
        model = self._order_from_v2_ack(
            response,
            ticker=ticker,
            status=self._v2_status_from_ack(response),
    ...
    async def decrease_order(
        ...
        if subaccount is not None:
            endpoint += f"?subaccount={subaccount}"
        response = await self._client.post(endpoint, {"reduce_by": reduce_by_fp})
        model = self._order_from_v2_ack(
            response,
    ...
    async def create_subaccount(
        ...
        response = await self._client.post("/portfolio/subaccounts", body)
    ...
    async def transfer_between_subaccounts(
        ...
        *,
        ...
        await self._client.post("/portfolio/subaccounts/transfer", body)
    ...
    async def get_subaccount_netting(self) -> DataFrameList[SubaccountNettingModel]:
        """Get the netting-enabled settings for all subaccounts."""
        response = await self._client.get("/portfolio/subaccounts/netting")
        return DataFrameList(
    ...
    def _build_order_data(
        ...
        *,
        ...
        if isinstance(price_ranges, (list, tuple)) and price_ranges:
            AsyncPortfolio._validate_price_ranges(yes_price, price_ranges)
